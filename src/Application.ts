...
export class Application extends EventEmitter {
	 ...
	 */
	public async load() {
		await this.initialiseDirectory(this.options.componentDir);
		await this.initialiseDirectory(this.options.serviceDir);
	...
	private async injectRefs() {
		...
		while ((ref = this.refs.shift())) {
			...
			Object.defineProperty(ref.target, ref.key, {
				get() { return f; },
				set(_) { throw new SyntaxError("References cannot mutate state"); }
			});
		}
	 ...
	 * @private
	 */
	private async initialiseDirectory(dir: string) {
		...
		for (const r of pending.sort((a, b) => a.priority - b.priority)) {
			r.ref = new r.ref();
			switch (r.type) {
				case "component": this.components.push(r); break;
				case "service": this.services.push(r); break;
			}
		}
	}
